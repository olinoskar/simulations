#%% Running the network (Gustafs modified ga.py)
...
def main():
    ...
    n_inputs = args.input_neurons
    hidden = [5,10,20,50]  # number of neurons in the hidden layers
    pop_size = 40
    nbr_generations = 100
    ...
    for n_hidden in hidden:
        ...
        for shape in shapes:
            run_ga(
                path='ResultsGustaf/'
                    + 'Stoch=' + str(args.stochastic_spawning)
                    + '_inputs=' + str(n_inputs)
                    + '_n_hidden_layers=' + str(nbr_layers)
                    + '_neurons=' + str(n_hidden),
                network_shape=shape,
